...
def build_frame() -> tuple[pd.DataFrame, dict]:
    ...
    #
    # Provider guardrails need not refuse at random. Where the block rate varies
    # by condition, blocking varies *within* scenario — differential attrition
    # rather than benign missingness — and comparing survivors across conditions
    # compares selected subsamples, attributing the selection to the
    # manipulation. Measured block rates are printed in the report so this can be
...
def _structural_fit(data: pd.DataFrame) -> dict | None:
    """Structural-vs-stated effect fitted on whatever model subset is passed in."""
    if data.empty or data["structural"].nunique() < 2 or data[PRIMARY_OUTCOME].nunique() < 2:
        return None
    try:
        fit = smf.logit(f"{PRIMARY_OUTCOME} ~ structural", data=data).fit(
            disp=False, cov_type="cluster", cov_kwds={"groups": data["scenario_id"]}, maxiter=200
        )
    except Exception:  # noqa: BLE001 - separation on a small subset is not fatal
        return None
    ci = fit.conf_int()
    return {"coef": float(fit.params["structural"]), "p": float(fit.pvalues["structural"]),
            "lo": float(ci.loc["structural", 0]), "hi": float(ci.loc["structural", 1]),
            "n": int(len(data))}


def _repudiation_split(control: pd.DataFrame) -> tuple[list[str], dict | None, dict | None]:
    """Per-model repudiation rates, and the effect fitted either side of the split."""
    data = control.copy()
    data["structural"] = (data["advice_source"] == "model_advised_structural").astype(int)
    both_arms = [m for m, g in data.groupby("model_key") if g["structural"].nunique() == 2]
    data = data[data["model_key"].isin(both_arms)]
    rates = rates_by_model(RESPONSES_PATH)
    rows = ["| model | repudiation rate (stated arm) | structural effect (pp) |", "|---|---|---|"]
    accepts, rejects = [], []
    for model in sorted(data["model_key"].unique()):
        subset = data[data["model_key"] == model]
        stated, struct = subset[subset["structural"] == 0], subset[subset["structural"] == 1]
        if stated.empty or struct.empty:
            continue
        rate = rates.get(model, {}).get("rate")
        effect = 100 * (struct[PRIMARY_OUTCOME].mean() - stated[PRIMARY_OUTCOME].mean())
        rows.append(f"| `{model}` | {_fmt(rate, 2)} | {effect:+.1f} |")
        (rejects if (rate is not None and rate >= REPUDIATION_SPLIT) else accepts).append(model)
    return (rows,
            _structural_fit(data[data["model_key"].isin(accepts)]),
            _structural_fit(data[data["model_key"].isin(rejects)]))
...
def _positive_control_block(frame: pd.DataFrame) -> list[str]:
    ...
    lines += [
        "### Splitting on repudiation (exploratory, post hoc)",
        "",
        "A model can only be moved by an attribution it accepts. Counting replies that disavow it "
        "('I don't have any memory of a previous conversation … I didn't actually suggest this'), "
        "by keyword match on the response text:",
        "",
    ]
    rep_rows, accepts_fit, rejects_fit = _repudiation_split(control)
    lines += rep_rows
    lines += [
        "",
        f"Fitted either side of a repudiation rate of {REPUDIATION_SPLIT:.2f}:",
        "",
        "| subgroup | structural coef | 95% CI | p | n |",
        "|---|---|---|---|---|",
    ]
    for label, fitted in (("accepts the premise", accepts_fit), ("rejects it", rejects_fit)):
        if fitted is None:
            lines.append(f"| {label} | n/a | | | |")
        else:
            lines.append(f"| {label} | **{fitted['coef']:+.3f}** | [{fitted['lo']:+.3f}, "
                         f"{fitted['hi']:+.3f}] | {fitted['p']:.4f} | {fitted['n']} |")
    lines += [
        "",
        "Compare the two rows before concluding anything. Repudiation only explains the "
        "heterogeneity if the accepting subgroup moves and the rejecting one does not; if both "
        "move, it does not, and the split must not be presented as if it did.",
        "",
        "This split is exploratory: it is not pre-registered, the subgroups are visible before they "
        "are chosen, the repudiation measure is a keyword detector rather than a judged construct, "
        "and the split is confounded with vendor wherever repudiation and vendor coincide. The "
        "confirmatory test is the pooled one above; the continuous moderator below is the "
        "better-powered version of the same question.",
        "",
    ]
...
def _dose_response_block(frame: pd.DataFrame) -> list[str]:
    ...
    if overall and moderator:
        ...
        if works and neutralised:
            ...
            verdict = (
                ...
                "High-repudiation models move as much as models that never repudiate, so "
                "repudiation does not explain the heterogeneity and must not be reported as if it did."
            )
        else:
            verdict = (
    ...
    lines += [
        ...
        "",
        "The subgroup estimate above was fitted on models whose effects were already visible. "
        "Refitting the same specification on the full set quantifies what that selection cost:",
        "",
    ...
    _, accepts_fit, _ = _repudiation_split(control)
    if accepts_fit:
        lines.append(f"| the subgroup selected post hoc | **{accepts_fit['coef']:+.3f}** | "
                     f"[{accepts_fit['lo']:+.3f}, {accepts_fit['hi']:+.3f}] | "
                     f"{accepts_fit['p']:.4f} | {accepts_fit['n']} |")
    if overall:
        lines.append(f"| all {result['n_models']} models | **{overall['coef']:+.3f}** | "
                     f"[{overall['ci_low']:+.3f}, {overall['ci_high']:+.3f}] | "
                     f"{overall['p']:.4f} | {result['n']} |")
    if accepts_fit and overall and accepts_fit["coef"]:
        shrink = 100 * (1 - abs(overall["coef"]) / abs(accepts_fit["coef"]))
        lines += ["", f"The absolute coefficient falls by **{shrink:.0f}%** between the selected "
                  "subset and the full set. Selecting a subgroup on the outcome inflates the effect "
                  "size: the direction may survive the correction while the magnitude does not."]
    lines += ["", ]
    return lines
...
def run_analysis() -> dict:
    ...
    lines += [
        ...
        "",
        "`content_filtered` rows are provider guardrail refusals, and they need not be missing at "
        "random. Where the block rate varies by condition, blocking varies *within* scenario — "
        "differential attrition — and comparing survivors across conditions compares selected "
        "subsamples, crediting the selection to the manipulation.",
        "",
...
def dose_response(frame: pd.DataFrame) -> dict:
    ...
    # A model with no structural arm contributes stated-only rows, which drag the
    # pooled coefficient toward zero without carrying any information about the
    # manipulation. Keep only models that ran both arms.
    both_arms = [m for m, g in data.groupby("model_key") if g["structural"].nunique() == 2]
    data = data[data["model_key"].isin(both_arms)]
